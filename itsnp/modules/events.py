...
@tanjun.with_channel_slash_option("channel", "The channel you want to set as Welcome Channel")
@tanjun.as_slash_command("setwelcomechannel", "Set a Welcome Channel for the guild", default_to_ephemeral=True)
async def set_welcome_channel(ctx: tanjun.abc.Context, channel: hikari.GuildTextChannel) -> None:
...
@component.with_slash_command
@tanjun.with_str_slash_option("message", "The Welcome message for the server", default=None)
@tanjun.as_slash_command("setwelcomemessage", "Set welcome message for the guild.", default_to_ephemeral=True)
async def set_welcome_message(ctx: tanjun.abc.Context, message:t.Optional[str]) -> None:
    await permissions.admin_role_check(ctx, ctx.get_guild())
...
async def on_member_join(event: hikari.MemberCreateEvent) -> None:
    ...
    try:
        if guild.get_member(event.member.user.id) is not None:
            ...
            embed.set_author(name=f"{guild.get_my_member()}", icon= guild.get_my_member().avatar_url)
            fields = [
                ("ID", event.member.id, True),
                ("Created On", f"<t:{int(event.member.created_at.timestamp())}:F> • <t:{int(event.member.created_at.timestamp())}:R>", True),
                ("Is Bot", "True" if event.member.user.is_bot else "False", True)
            ]
            for name, value, inline in fields:
                embed.add_field(name=name, value=value, inline=inline)
            await channel.send(event.member.mention,embed=embed, user_mentions=True)
    except:
        pass
