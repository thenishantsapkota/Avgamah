...
@tanjun.as_slash_command("link", "Show different avgamah link")
...
@tanjun.as_slash_command("links", "Get all avgamah Links")
async def links_command(ctx: tanjun.abc.Context) -> None:
    ...
    for i in range(0, len(PLATFORM), 2):
        ...
        rows.append(row)
    await ctx.respond("**Here are all the avgamah Links**", components=rows)
